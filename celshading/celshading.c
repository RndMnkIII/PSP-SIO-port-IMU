...
PSP_MODULE_INFO("Celshading", 0x1000, 1, 1);
...
//union  fltbarray KalmanX;
//union  fltbarray KalmanY;
struct sensorIMU simu;

int leerFloat(struct sensorIMU *sensor)
{
    int 	ch;
    int count=0;
    int inicio_datos=0;
    while(count <= 7)
    {
        ...
        if(ch>=0){
            if(ch == 0xD){ //detectamos caracter de inicio de recepcion de datos>
                if(inicio_datos == 1) return -1; //Error: ya habiamos empezado a recibir dato y no se ha completado recepcion
                count=0;
                inicio_datos=1;
            }
            else{ //se estan recibiendo datos
                if(inicio_datos == 1){ //no se leen datos hasta haber detectado inicio de recepcion de datos previamente
                    if (count > 3) sensor->y.bval[count-4]=(uint8_t) ch;
                    else sensor->x.bval[count]=(uint8_t) ch;
                    count++;
                }
            }
        }
...
int main(int argc, char* argv[]) {
	...
	while(running())
	{
        ...
        estado=leerFloat(&simu);
        if (estado == 0) {
            if(simu.x.fval > 90.f) simu.x.fval = 90.0f;
            if(simu.y.fval > 90.f) simu.y.fval = 90.0f;
            tx = simu.x.fval / 90.0f;
            ty = simu.y.fval / 90.0f;
            //pspDebugScreenPrintf("Kalman X:%.1f Y:%.1f\n", simu.x.fval, simu.y.fval);
        }
		sceGuStart(GU_DIRECT,list);
		...
		*/
		...
		//tx = (pad.Lx - 128) / 127.0f;
		//ty = (pad.Ly - 128) / 127.0f;
		if(fabs(tx) <= 0.2f) {
			tx = 0.0f;
		} else {
			if(tx > 0.0f) {
